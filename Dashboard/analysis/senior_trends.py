...
def get_top10_trends(db_path="../timekeeping.db", start_date="2010-01-01"):
    ...
    def build_monthly(eid):
        ...
        m = mb.rename('billable').to_frame().join(mn.rename('nonbillable'),
        ...
        # Build m_idx
        m['month_idx'] = m['month'].astype('period[M]').astype('int64').astype(int)
        m['m_idx'] = m['month_idx'] - m['month_idx'].min()

        # Center time around its mean
        mean_idx = m['m_idx'].mean()
        x_centered = m['m_idx'] - mean_idx

        # Regression on raw pct (no smoothing)
